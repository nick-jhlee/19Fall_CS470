...
ckpt = ModelCheckpoint(filepath='./dataset/ckpt/LSTM_adam--{epoch:02d}--{val_loss:.3f}.hdf5', monitor='val_accuracy', verbose=1, mode='max', save_best_only=True)
#ckpt = ModelCheckpoint(filepath='./dataset/ckpt/mel_LSTM_ada--{epoch:02d}--{val_loss:.3f}.hdf5', monitor='val_loss', verbose=1, mode='min', period=1)
opt = Adam()

##Save model structure as json file
model_json=model.to_json()
with open("./dataset/ckpt/LSTM_adam.json", "w") as json_file:
...
pandas.DataFrame(history.history).to_csv("LSTM_training_history.csv")

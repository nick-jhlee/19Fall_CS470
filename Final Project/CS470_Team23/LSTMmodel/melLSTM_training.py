...
print("Valid X shape: " + str(genre_features.valid_X.shape))
print("Valid Y shape: " + str(genre_features.valid_Y.shape))
...
ckpt = ModelCheckpoint(filepath='./dataset/ckpt/mel_LSTM_ada--{epoch:02d}--{val_loss:.3f}.hdf5', monitor='val_accuracy', verbose=1, mode='max', save_best_only=True)
...
with open("./dataset/ckpt/melLSTM_adam.json", "w") as json_file:
...
pandas.DataFrame(history.history).to_csv("melLSTM_training_history.csv")
